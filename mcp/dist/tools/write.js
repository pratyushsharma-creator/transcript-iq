...
export async function createBlogPost(input) {
    ...
    const data = { title, contentType };
    if (excerpt !== undefined)
        data.excerpt = excerpt;
    if (featured !== undefined)
        data.featured = featured;
    if (bodyText)
        data.body = textToLexical(bodyText);
    const result = await payloadCreate('blog-posts', data);
...
export async function updateBlogPost(input) {
    ...
    if (rest.excerpt !== undefined)
        data.excerpt = rest.excerpt;
    if (rest.contentType !== undefined)
        data.contentType = rest.contentType;
    if (rest.featured !== undefined)
        data.featured = rest.featured;
    if (bodyText)
        data.body = textToLexical(bodyText);
    await payloadPatch('blog-posts', id, data);
...
export async function createTranscript(input) {
    ...
    const result = await payloadCreate('expert-transcripts', data);
    const id = result?.doc?.id ?? result?.id ?? 'unknown';
...
export async function updateTranscript(input) {
    ...
    await payloadPatch('expert-transcripts', id, data);
...
export async function createEarnings(input) {
    ...
    const result = await payloadCreate('earnings-analyses', data);
    const id = result?.doc?.id ?? result?.id ?? 'unknown';
...
export async function updateEarnings(input) {
    ...
    await payloadPatch('earnings-analyses', id, data);
...
export async function publishContent(input) {
    ...
    await payloadPatch(collection, id, { _status: 'published' });
...
export async function unpublishContent(input) {
    const { collection, id } = input;
    await payloadPatch(collection, id, { _status: 'draft' });
