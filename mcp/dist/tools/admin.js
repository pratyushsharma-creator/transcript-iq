...
export async function draftContent(args) {
    const result = await apiPost('/api/ai/draft', { brief: args.brief, type: args.type }, true);
    if (result.error) {
...
export async function generateMeta(args) {
    const result = await apiPost('/api/ai/meta', { body: args.body, title: args.title, type: args.type }, true);
    if (result.error) {
...
export async function suggestTags(args) {
    const result = await apiPost('/api/ai/tags', { body: args.body, type: args.type }, true);
    if (result.error) {
