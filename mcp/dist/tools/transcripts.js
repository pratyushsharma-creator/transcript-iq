import { z } from 'zod';
import { apiGet, buildPayloadQuery } from '../api-client.js';
// ── Tool schemas ──────────────────────────────────────────────────────────────
...
// ── Tool handlers ─────────────────────────────────────────────────────────────
export async function listTranscripts(args) {
    const where = { _status: 'published' };
    if (args.tier)
        where.tier = args.tier;
    if (args.geography)
        where.geography = args.geography;
    ...
    const result = await apiGet('/api/expert-transcripts', params);
    ...
    for (const t of result.docs) {
        ...
        if (t.expertFormerTitle)
            lines.push(`  Expert: ${t.expertFormerTitle}`);
