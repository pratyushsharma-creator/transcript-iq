      ...
      <div className={`mt-8 grid gap-x-10 gap-y-6 grid-cols-2 sm:grid-cols-3 lg:grid-cols-${cols} items-center`}>
        {(block.logos ?? []).map((logo, i) => {
          const inner = logo.image?.url ? (
            <Image src={logo.image.url} alt={logo.image.alt ?? logo.name} width={(logo.image as { url: string; alt?: string; width?: number }).width ?? 120} height={32} className="h-8 w-auto object-contain" />
          ) : (
